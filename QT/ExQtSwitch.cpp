...
{
    if( target.isNull() == true && oldState == Stopped )
    {
    ...
    if( endValue().isValid() == false && direction() == Forward )
    {
...
{
    return isChecked() == true ? Qt::Checked : Qt::Unchecked;
}
...
QRect Switch::indicatorRect()
{
    ...
    return ltr( this ) == true ? QRect( 0, 0, w, style.height ) : QRect( width() - w, 0, w, style.height );
}

QRect Switch::textRect()
{
    const auto w = style.indicatorMargin.left() + style.height + style.indicatorMargin.right();
    return ltr( this ) == true ? rect().marginsRemoved( QMargins( w, 0, 0, 0 ) ) : rect().marginsRemoved( QMargins( 0, 0, w, 0 ) );
}
...
{
    auto h = style.height;
    // NOTE : 해당부분 fontMetrics().width( text() ) 부분이 QT 5.X 버전 코드여서 boundingRect( text() ).width()로 변경함. 문제 있으면 수정 필요
    auto w = style.indicatorMargin.left() + style.height + style.indicatorMargin.right() + fontMetrics().boundingRect( text() ).width();
...
{
    shadowPixmap = Ex::Qt::Style::drawShadowEllipse( THUMB_RADIUS, SHADOW_ELEVATION, QColor( 0, 0, 0, 70 ) );
