    ...
    if (hSCM) {
        ...
        if (hService) {
            ...
            if (pQueryServiceConfig(hService, (LPQUERY_SERVICE_CONFIG)data, 8 * 1024, &sizeNeeded)) {
                ...
                // TODO : ushort -> char16_t >> 정상 데이터 표기 확인 필요함
                const ushort* bPath = ( const ushort* ) config->lpBinaryPathName;
                const char16_t* newData = reinterpret_cast< const char16_t* >( bPath );
                result = QString::fromUtf16( newData );
            }
            pCloseServiceHandle(hService);
        }
        pCloseServiceHandle(hSCM);
    ...
    if (hSCM) {
        ...
        if (hService) {
            ...
            if (pQueryServiceConfig2(
                ...
                {
                    const ushort* bPath = ( const ushort* ) desc->lpDescription;
                    const char16_t* newData = reinterpret_cast< const char16_t* >( bPath );
                    result = QString::fromUtf16( newData );
                }
            }
...
void WINAPI QtServiceSysPrivate::serviceMain(DWORD dwArgc, wchar_t** lpszArgv)
{
    ...
    {
        const unsigned short* lpsz = ( unsigned short* ) lpszArgv[ i ];
        const char16_t* newData = reinterpret_cast< const char16_t* >( lpsz );
        instance->serviceArgs.append( QString::fromUtf16( newData ) );
    }
...
QString QtServiceBasePrivate::filePath() const
{
    ...
    const unsigned short* oPath = ( unsigned short* ) path;
    const char16_t* newData = reinterpret_cast< const char16_t* >( oPath );

    return QString::fromUtf16( newData );
}
